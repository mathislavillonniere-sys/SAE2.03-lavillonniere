...
MovieDetail.format = function (film, isFav) {
  ...
  // SI DÉJÀ EN FAVORI : On affiche le bouton "Retirer"
  if (isFav == 1) {
    html = html.replace(
      "{{favoris}}",
      `<button class="btn-remove" onclick="C.handlerDeleteFavoris(${film.id})">Retirer des favoris</button>`,
    );
  } else {
    // SINON : On affiche le bouton "Ajouter"
    html = html.replace(
      "{{favoris}}",
